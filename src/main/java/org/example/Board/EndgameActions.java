...
public class EndgameActions {
    ...
    public EndgameActions(int[] kingPosition, Board board) {
    ...
    public boolean canFigureAttackEnemy(int[] enemyFigure, Object playerSymbol) {
        ...
        for (int y = 0; y <= 7; y++) {
            for (int x = 0; x <= 7; x++) {
                ...
                boolean canFigureAttackEnemyFigure = b[y][x] instanceof Figure
                                                     && ((Figure) b[y][x]).getSymbol().getClass() == playerSymbol.getClass()
                                                     && ((Figure) b[y][x]).canMove(new int[]{y, x}, enemyFigure, board)
                                                     && !between.isFigureInBetween();
                if (canFigureAttackEnemyFigure) {
                    boolean isAllyFigureAKing = b[y][x] instanceof King;
                    if (isAllyFigureAKing) {
                        ...
                        for (List<Integer> figure : listOfEnemyFigures) {
                            int u = figure.get(0);
                            int v = figure.get(1);
                            Between between2 = new Between(new int[]{u, v}, new int[]{enemyFigure[0], enemyFigure[1]}, board.getBoard());
                            boolean canEnemyAttackKingAfterTakingEnemyFigure = b[y][x] instanceof Figure
                                                                               && !asList(u, v).equals(asList(enemyFigure[0], enemyFigure[1]))
                                                                               && ((Figure) b[u][v]).canMove(new int[]{u, v}, new int[]{enemyFigure[0], enemyFigure[1]}, board)
                                                                               && !between2.isFigureInBetween();
                            if (canEnemyAttackKingAfterTakingEnemyFigure) {
                                return false;
                            }
                        }
                    }
    ...
    public int[] getFigureTargetingKingPos(Object playerSymbol) {
        ...
        for (int y = 0; y <= 7; y++) {
            for (int x = 0; x <= 7; x++) {
                ...
                boolean canEnemyFigureMoveToKing = b[y][x] instanceof Figure
                                                   && ((Figure) b[y][x]).getSymbol().getClass() != playerSymbol.getClass()
                                                   && ((Figure) b[y][x]).canMove(new int[]{y, x}, kingPosition, board)
                                                   && !between.isFigureInBetween();
                if (canEnemyFigureMoveToKing) {
                    return new int[]{y, x};
                }
            }
        }
